...
unordered_map<string, int> TextSimilarity::getWordFreq(const char* text)  //得到词频
{
	string line;
...
double TextSimilarity::GetCosine(const char* txt1, const char* txt2)  //将所有函数封装成一个接口，求两个文档的文本相似度
{
    ...
    vector<pair<string, int>> v1 = sortByValueReverse(wfg1);  //将文档1得到的词频进行排序
    vector<pair<string, int>> v2 = sortByValueReverse(wfg2);  //将文档2得到的词频进行排序

	unordered_set<string> wset1;  //用来存文档1中排序好的元素
	unordered_set<string> wset2;  //用来存文档2中排序号的元素
    selectAimWords(v1, wset1);
    selectAimWords(v2, wset2); //这两个函数用来将vector中的元素存到set中，构建词频向量


	vector<double> vd1;
	vector<double> vd2;
    vd1 = getOneHot(wset1, wfg1);  //拿到文档1的构建好的词频向量
    vd2 = getOneHot(wset2, wfg2); //拿到文档2的构建好的词频向量
